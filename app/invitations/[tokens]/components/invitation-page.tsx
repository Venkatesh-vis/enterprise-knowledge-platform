...
export function InvitationPage({
  ...
  const valid =
    ...
    data.state === "valid-existing-user";
    ...
    <main className="flex min-h-screen items-center justify-center bg-[#f8faff] px-4 py-10">
      <div className="w-full max-w-lg">
        <div className="mb-8 flex justify-center">
          ...
          >
            <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-indigo-600 via-violet-600 to-blue-600 text-white">
          ...
          {valid ? (
            <div className="p-6 sm:p-8">
              ...
              {data.state === "valid-new-user" ? (
                ...
                <div className="mt-6">
                  <p className="rounded-xl border border-indigo-100 bg-indigo-50 p-4 text-sm text-indigo-800">
                  ...
                  {data.authenticated ? (
                    <button
                      ...
                      disabled={loading}
                      className="mt-4 w-full rounded-lg bg-slate-950 px-4 py-2.5 text-sm font-medium text-white disabled:opacity-50"
                    >
                      {loading
                        ? "Joining..."
                        : "Accept invitation"}
                    ...
                    <Link
                      ...
                      className="mt-4 flex w-full justify-center rounded-lg bg-slate-950 px-4 py-2.5 text-sm font-medium text-white"
                    >
                      Sign in & accept
                    </Link>
                  )}
...
> = {
  ...
  "already-member": {
    ...
    description:
      "This account already belongs to this organization.",
    className:
      "bg-emerald-50 text-emerald-600",
  },
  "already-in-another-organization": {
    icon: AlertCircle,
    title: "Account already belongs elsewhere",
    description:
      "This account is already associated with another organization.",
    className:
  ...
  error: {
    ...
    description:
      "The invitation could not be loaded. Try again later.",
    className:
      "bg-red-50 text-red-600",
  },
};
...
function Status({
  ...
  state: PublicInvitationData["state"];
}) {
  const config = STATUS[
    state as keyof typeof STATUS
  ] ?? STATUS.invalid;
  ...
  return (
    <div className="p-8 text-center">
      <div
        className={`mx-auto flex h-12 w-12 items-center justify-center rounded-xl ${config.className}`}
      >
      ...
      <Link
        href="/"
        className="mt-6 inline-flex rounded-lg border border-slate-200 px-4 py-2.5 text-sm font-medium text-slate-700"
      >
        Return home
      </Link>
    </div>
