...
export default async function DocumentsPage({
  ...
  return (
    <div className="space-y-8">
      <DocumentsHeader totalDocuments={data.pagination.totalItems} />
      <DocumentsToolbar />
      <section className="space-y-4">
        <div className="flex items-center justify-between gap-4">
          <p className="text-sm text-slate-500">
            Showing <span className="font-medium text-slate-700">{data.documents.length}</span> of <span className="font-medium text-slate-700">{data.pagination.totalItems}</span> documents
          </p>
        </div>
